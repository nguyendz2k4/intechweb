...
import { deleteAdminService, getAdminServices, saveAdminService } from '../../../lib/admin-store'
...
export async function GET() {
  return NextResponse.json(await getAdminServices())
}
...
export async function POST(request: Request) {
  ...
  return NextResponse.json(await saveAdminService(service), { status: 201 })
}
...
export async function DELETE(request: Request) {
  ...
  return NextResponse.json(await deleteAdminService(slug))
}
