    ...
    <main>
      ...
      <section className="section surface">
        <div className="spec-grid">
          <div>
            <p className="eyebrow">Thông số tư vấn</p>
            <h2>Điểm cần kiểm tra trước khi chọn thiết bị</h2>
            <ul className="list">{(product.specs ?? []).map((spec) => <li key={spec}>{spec}</li>)}</ul>
          </div>
          <div>
            <p className="eyebrow">Ứng dụng</p>
            <h2>Phù hợp cho các nhu cầu triển khai</h2>
            <ul className="list">{(product.applications ?? []).map((item) => <li key={item}>{item}</li>)}</ul>
          </div>
        </div>
      </section>
      <section className="section">
        <div className="quote-box">
          <p className="eyebrow">Tư vấn cấu hình</p>
          <h2>Không chắc model này có phù hợp không?</h2>
          <p>Gửi Intech hiện trạng số lượng người dùng, mặt bằng, thiết bị đang có và mục tiêu triển khai. Intech sẽ gợi ý cấu hình phù hợp hơn thay vì chỉ bán đúng một mã sản phẩm.</p>
          <div className="actions">
            <Link className="btn primary" href={`tel:${siteConfig.phone}`}>Gọi {siteConfig.phoneDisplay}</Link>
            <Link className="btn secondary" href="/lien-he">Gửi yêu cầu tư vấn</Link>
          </div>
        </div>
      </section>
      <CTA />
